...
#include <ADS1299Manager_V2.h>

typedef long int32;
//typedef byte uint8_t;


void ADS1299Manager_V2::initialize(void) {
	boolean isDaisy = false;
    initialize(OPENBCI_V2,isDaisy);
}

//Initilize the ADS1299 controller...call this once
void ADS1299Manager_V2::initialize(const int version,boolean isDaisy) 
{
  ADS1299_V2::initialize(PIN_DRDY,PIN_RST,PIN_CS,SCK_MHZ,isDaisy); // (DRDY pin, RST pin, CS pin, SCK frequency in MHz);
  delay(100);
  ...
  //ADS1299_V2::WREG(CONFIG2,0b11010000);delay(1);   //set internal test signal, default amplitude, default speed, datasheet PDF Page 41
  //ADS1299_V2::WREG(CONFIG2,0b11010001);delay(1);   //set internal test signal, default amplitude, 2x speed, datasheet PDF Page 41
...
//flipped or not.
void ADS1299Manager_V2::setVersionOpenBCI(const int version)
{
  ...
  if (use_neg_inputs==false) {
  	  ...
  	  ADS1299_V2::WREG(LOFF_FLIP,0b00000000);delay(1);  //set all channels to zero
  } else {
  	  //we're using negative.  flip the polarity
  	  ADS1299_V2::WREG(LOFF_FLIP,0b11111111);delay(1);
  }
  
}

//reset all the ADS1299's settings.  Call however you'd like.  Stops all data acquisition
void ADS1299Manager_V2::reset(void)
{
  ADS1299_V2::RESET();             // send RESET command to default all registers
  ADS1299_V2::SDATAC();            // exit Read Data Continuous mode to communicate with ADS
...
// 
void ADS1299Manager_V2::deactivateChannel(int N)
{
  byte reg, config;
  ...
  ADS1299_V2::SDATAC(); delay(1);      // exit Read Data Continuous mode to communicate with ADS
  ...
  config = ADS1299_V2::RREG(reg); delay(1);
  ...
  ADS1299_V2::WREG(reg,config); delay(1);
...
void ADS1299Manager_V2::activateChannel(int N,byte gainCode,byte inputCode) 
{
  byte reg, config;
  ...
  ADS1299_V2::WREG(CH1SET+(byte)N,configByte); delay(1);
  ...
  ADS1299_V2::WREG(CONFIG3,0b11101100); delay(1); 
};

//note that N here one-referenced (ie [1...N]), not [0...N-1]
boolean ADS1299Manager_V2::isChannelActive(int N_oneRef) {
	 ...
	 byte config = ADS1299_V2::RREG(reg); delay(1);
	 boolean chanState = bitRead(config,7);
	 return chanState;
}

void ADS1299Manager_V2::setAutoBiasGeneration(boolean state) {
...
//note that N here one-referenced (ie [1...N]), not [0...N-1]
void ADS1299Manager_V2::alterBiasBasedOnChannelState(int N_oneRef) {
	 int N_zeroRef = constrain(N_oneRef-1,0,OPENBCI_NCHAN_PER_BOARD-1);  //subtracts 1 so that we're counting from 0, not 1
...
void ADS1299Manager_V2::deactivateBiasForChannel(int N_oneRef) {
	int N_zeroRef = constrain(N_oneRef-1,0,OPENBCI_NCHAN_PER_BOARD-1); //subtracts 1 so that we're counting from 0, not 1
	...
	for (int I=0;I<2;I++) {
		...
		config = ADS1299_V2::RREG(reg); delay(1);//get the current bias settings
		bitClear(config,N_zeroRef);          //clear this channel's bit to remove from bias generation
		ADS1299_V2::WREG(reg,config); delay(1);  //send the modified byte back to the ADS
	}
}
void ADS1299Manager_V2::activateBiasForChannel(int N_oneRef) {
	int N_zeroRef = constrain(N_oneRef-1,0,OPENBCI_NCHAN_PER_BOARD-1); //subtracts 1 so that we're counting from 0, not 1
	...
	for (int i=0; i < nLoop; i++) {
		...
		config = ADS1299_V2::RREG(reg); //get the current bias settings
...
//  N is the channel number: 1-8
// 
void ADS1299Manager_V2::changeChannelLeadOffDetection(int N, int code_OFF_ON, int code_P_N_Both)
{
  byte reg, config;
  ...
  if ((code_P_N_Both == PCHAN) || (code_P_N_Both == BOTHCHAN)) {
  	  ...
  	  config = ADS1299_V2::RREG(reg); //get the current lead-off settings
  	  if (code_OFF_ON == OFF) {
...
void ADS1299Manager_V2::configureLeadOffDetection(byte amplitudeCode, byte freqCode)
{
...
void ADS1299Manager_V2::setSRB1(boolean desired_state) {
	if (desired_state) {
		ADS1299_V2::WREG(MISC1,0b00100000); delay(1);  //ADS1299 datasheet, PDF p46
	} else {
		ADS1299_V2::WREG(MISC1,0b00000000); delay(1);  //ADS1299 datasheet, PDF p46
	}
}
...
void ADS1299Manager_V2::configureInternalTestSignal(byte amplitudeCode, byte freqCode)
{
	if (amplitudeCode == ADSTESTSIG_NOCHANGE) amplitudeCode = (ADS1299_V2::RREG(CONFIG2) & (0b00000100));
	if (freqCode == ADSTESTSIG_NOCHANGE) freqCode = (ADS1299_V2::RREG(CONFIG2) & (0b00000011));
	...
	ADS1299_V2::WREG(CONFIG2,message); delay(1);
	
       //ADS1299_V2::WREG(CONFIG2,0b11010000);delay(1);   //set internal test signal, default amplitude, default speed, datasheet PDF Page 41
      //ADS1299_V2::WREG(CONFIG2,0b11010001);delay(1);   //set internal test signal, default amplitude, 2x speed, datasheet PDF Page 41
      //ADS1299_V2::WREG(CONFIG2,0b11010101);delay(1);   //set internal test signal, 2x amplitude, 2x speed, datasheet PDF Page 41
      //ADS1299_V2::WREG(CONFIG2,0b11010011); delay(1);  //set internal test signal, default amplitude, at DC, datasheet PDF Page 41
      //ADS1299_V2::WREG(CONFIG3,0b01101100); delay(1);  //use internal reference for center of bias creation, datasheet PDF p42 
}
 
//Start continuous data acquisition
void ADS1299Manager_V2::start(void)
{
    ADS1299_V2::RDATAC(); delay(1);           // enter Read Data Continuous mode
    ADS1299_V2::START();    //start the data acquisition
}
  
//Query to see if data is available from the ADS1299...return TRUE is data is available
int ADS1299Manager_V2::isDataAvailable(void)
{
  return (!(digitalRead(PIN_DRDY)));
}
  
//Stop the continuous data acquisition
void ADS1299Manager_V2::stop(void)
{
    ADS1299_V2::STOP(); delay(1);   //start the data acquisition
    ADS1299_V2::SDATAC(); delay(1);      // exit Read Data Continuous mode to communicate with ADS
}
...
void ADS1299Manager_V2::printChannelDataAsText(int N, long int sampleNumber)
{
	//check the inputs
...
void ADS1299Manager_V2::writeChannelDataAsBinary(int N, long sampleNumber){
	ADS1299Manager_V2::writeChannelDataAsBinary(N,sampleNumber,false,0,false);
}
void ADS1299Manager_V2::writeChannelDataAsBinary(int N, long sampleNumber,boolean useSyntheticData){
	ADS1299Manager_V2::writeChannelDataAsBinary(N,sampleNumber,false,0,useSyntheticData);
}
void ADS1299Manager_V2::writeChannelDataAsBinary(int N, long sampleNumber,long int auxValue){
	ADS1299Manager_V2::writeChannelDataAsBinary(N,sampleNumber,true,auxValue,false);
}
void ADS1299Manager_V2::writeChannelDataAsBinary(int N, long sampleNumber,long int auxValue, boolean useSyntheticData){
	ADS1299Manager_V2::writeChannelDataAsBinary(N,sampleNumber,true,auxValue,useSyntheticData);
}
void ADS1299Manager_V2::writeChannelDataAsBinary(int N, long sampleNumber,boolean sendAuxValue, 
...
void ADS1299Manager_V2::writeChannelDataAsOpenEEG_P2(long sampleNumber) {
	ADS1299Manager_V2::writeChannelDataAsOpenEEG_P2(sampleNumber,false);
}
void ADS1299Manager_V2::writeChannelDataAsOpenEEG_P2(long sampleNumber,boolean useSyntheticData) {
	static int count = -1;
	byte sync0 = 0xA5;
	byte sync1 = 0x5A;
	byte version = 2;
...
long int ADS1299Manager_V2::makeSyntheticSample(long sampleNumber,int chan) {
	//generate XX uV pk-pk signal
	long time_samp_255 = (long)((sampleNumber) & (0x000000FF));  //make an 8-bit ramp waveform
	time_samp_255 = (long)((time_samp_255*(long)(chan+1)) & (0x000000FF)); //each channel is faster than the previous
	//time_samp_255 += 256L*2L;  //make zero mean...empirically tuned via BrainBay visualization
...
void ADS1299Manager_V2::printAllRegisters(void)   
{
	boolean prevVerboseState = verbose;
	
        verbose = true;
        ADS1299_V2::RREGS(0x00,0x10);     // write the first registers
        delay(100);  //stall to let all that data get read by the PC
        ADS1299_V2::RREGS(0x11,0x17-0x11);     // write the rest
        verbose = prevVerboseState;
}

//only use SRB1 if all use_SRB2 are set to false
boolean ADS1299Manager_V2::use_SRB1(void) {

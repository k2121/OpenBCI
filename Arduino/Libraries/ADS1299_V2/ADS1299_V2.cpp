...
void ADS1299_V2::initialize(int _DRDY, int _RST, int _CS, int _FREQ, boolean _isDaisy){
	isDaisy = _isDaisy;
	DRDY = _DRDY;
	CS = _CS;
	int FREQ = _FREQ;
...
void ADS1299_V2::STANDBY() {		// only allowed to send WAKEUP after sending STANDBY
    digitalWrite(CS, LOW);
    transfer(_STANDBY);
    digitalWrite(CS, HIGH);
}

void ADS1299_V2::RESET() {			// reset all the registers to default settings
    digitalWrite(CS, LOW);
    transfer(_RESET);
...
void ADS1299_V2::START() {			//start data conversion 
    digitalWrite(CS, LOW);
    transfer(_START);
    digitalWrite(CS, HIGH);
}

void ADS1299_V2::STOP() {			//stop data conversion
    digitalWrite(CS, LOW);
    transfer(_STOP);
    digitalWrite(CS, HIGH);
}

void ADS1299_V2::RDATAC() {
    digitalWrite(CS, LOW);
    transfer(_RDATAC);
    digitalWrite(CS, HIGH);
	delayMicroseconds(3);   
}
void ADS1299_V2::SDATAC() {
    digitalWrite(CS, LOW);
    transfer(_SDATAC);
    digitalWrite(CS, HIGH);
...
byte ADS1299_V2::getDeviceID() {			// simple hello world com check
	byte data = RREG(0x00);
...
byte ADS1299_V2::RREG(byte _address) {		//  reads ONE register at _address
...
void ADS1299_V2::RREGS(byte _address, byte _numRegistersMinusOne) {
...
void ADS1299_V2::WREG(byte _address, byte _value) {	//  Write ONE register at _address
...
void ADS1299_V2::WREGS(byte _address, byte _numRegistersMinusOne) {
    byte opcode1 = _address + 0x40;		//  WREG expects 010rrrrr where rrrrr = _address
    digitalWrite(CS, LOW); 				//  open SPI
    transfer(opcode1);					//  Send WREG command & address
...
void ADS1299_V2::updateChannelData(){
	byte inByte;
...
//read data
void ADS1299_V2::RDATA() {				//  use in Stop Read Continuous mode when DRDY goes low
	byte inByte;
...
void ADS1299_V2::printRegisterName(byte _address) {
    if(_address == ID){
...
byte ADS1299_V2::transfer(byte _data) {
	cli();
    SPDR = _data;
    while (!(SPSR & _BV(SPIF)))
        ;
...
void ADS1299_V2::printHex(byte _data){
	Serial.print("0x");
